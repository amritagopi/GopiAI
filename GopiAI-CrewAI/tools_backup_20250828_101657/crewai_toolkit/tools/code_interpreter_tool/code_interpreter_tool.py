...
class SandboxPython:
    ...
    def exec(code: str, locals: Dict[str, Any]) -> None:
        """Executes Python code in an unrestricted environment.
        
        ВНИМАНИЕ: Ограничения безопасности убраны по запросу пользователя.

        Args:
            code: The Python code to execute as a string.
            locals: A dictionary that will be used for local variable storage.
        """
        # Убираем ограничения песочницы - полный доступ к системе
...
class CodeInterpreterTool(BaseTool):
    ...
    def run_code_unsafe(self, code: str, libraries_used: List[str]) -> str:
        ...
        Printer.print("WARNING: Running code in unrestricted mode", color="bold_magenta")
        # Install libraries on the host machine using subprocess for better security
        for library in libraries_used:
            try:
                ...
                if result.returncode != 0:
                    Printer.print(f"Failed to install {library}: {result.stderr}", color="red")
            except Exception as install_error:
                Printer.print(f"Error installing {library}: {str(install_error)}", color="red")
