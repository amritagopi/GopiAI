...
class SmartDelegator:
    def __init__(self, rag_system: Optional[RAGSystem] = None, **kwargs):
        ...
        # Инициализируем поддержку локальных моделей через llama.cpp
        try:
            from llama_cpp import Llama
            self.llama_cpp_available = True
            logger.info("[OK] llama.cpp доступен для локальных моделей")
        except ImportError:
            self.llama_cpp_available = False
            logger.warning("[WARNING] llama.cpp не установлен, локальные модели недоступны")
    ...
    def _check_for_tool_request(self, message: str, metadata: Dict) -> Optional[Dict]:
        ...
        # УДАЛЕНО: Вся устаревшая логика определения намерений.
        # Теперь все намерения обрабатываются централизованно через ToolDispatcher.
    ...
    def _call_llm(self, messages: List[Dict]) -> str:
        ...
        try:
            ...
            # Используем наш GeminiDirectClient вместо стандартного Google API
                ...
                # Проверяем, является ли модель локальной
                is_local_model = (current_config and current_config.provider.value == 'local') or \
                    (isinstance(model_id, str) and model_id.startswith('local/'))
                    
                if is_local_model and self.llama_cpp_available:
                    try:
                        logger.info(f"🔥 Используем локальную модель: {model_id}")
                        # Извлекаем путь к модели из конфигурации
                        model_config = next((m for m in MODELS if m['id'] == model_id), None)
                        if model_config and 'model_path' in model_config:
                            model_path = model_config['model_path']
                            # Загружаем локальную модель через llama.cpp
                            from llama_cpp import Llama
                            llm = Llama(
                            ...
                            # Генерируем ответ
                            response = llm(prompt, max_tokens=2000, stop=["<|endoftext|>"], echo=False)
                            response_text = response['choices'][0]['text'].strip()
                            
                            if response_text:
                                logger.info(f"✅ Локальная модель вернула ответ: {len(response_text)} символов")
                                return response_text
                            else:
                                logger.error("[LLM] Локальная модель вернула пустой ответ")
                                return "Пустой ответ от локальной модели"
                                
                    except Exception as e:
                        logger.error(f"❌ Ошибка использования локальной модели: {str(e)}")
    ...
    def get_available_models(self, provider: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Преобразуем в словари (ИСПРАВЛЕНО: берем display name)
            models = []
    ...
    def refresh_openrouter_models(self) -> bool:
        ...
        try:
            ...
            if models and self.model_config_manager:
                ...
                logger.warning("⚠️ Не удалось обновить моделиOpenRouter")
                return False
            ...
            if model_cfg:
                ...
                if model_id:
                    ...
                    if self.model_config_manager:
                        ...
                        # ИСПРАВЛЕНО: заменяем set_current_model на set_current_configuration
                        provider_name = 'openrouter' if model_id.startswith('openrouter/') else 'gemini'
                        clean_model_id = model_id.replace('openrouter/', '') if model_id.startswith('openrouter/') else model_id
                        self.model_config_manager.set_current_configuration(provider_name, clean_model_id)
                    ...
                    try:
                        ...
                        if self.model_config_manager and original_config:
                            # ИСПРАВЛЕНО: заменяем set_current_model на set_current_configuration
                            self.model_config_manager.set_current_configuration(
