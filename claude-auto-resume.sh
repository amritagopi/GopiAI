...
extract_new_format_timestamp() {
    ...
    # Extract the reset time string (e.g., "6:30pm")
    reset_time_str=$(echo "$claude_output" | grep -oE 'resets [0-9]{1,2}:[0-9]{2}[ap]m' | awk '{print $2}')

    if [ -z "$reset_time_str" ]; then
        # Fallback for old format without minutes
        reset_time_str=$(echo "$claude_output" | grep -oE 'resets [0-9]{1,2}[ap]m' | awk '{print $2}')
        if [ -z "$reset_time_str" ]; then
            echo "[ERROR] Failed to extract reset time from new Claude output format."
            echo "[HINT] Expected format like: '...resets 6:30pm' or '...resets 6pm'"
            echo "[SUGGESTION] Check if Claude CLI output format has changed."
            echo "[DEBUG] Raw output: $claude_output"
            exit 2
        fi
    ...
    # Let `date` parse the time string directly, forcing C locale to handle am/pm
    now_timestamp=$(LC_ALL=C date +%s)
    today_reset=$(LC_ALL=C date -d "$reset_time_str" +%s 2>/dev/null)

    # Check if date command failed
    if [ $? -ne 0 ]; then
        echo "[ERROR] Failed to parse the extracted reset time: '$reset_time_str'"
        echo "[HINT] The 'date' command could not understand this time format."
        echo "[SUGGESTION] This might be a bug in the script's time parsing logic."
        exit 2
    fi
    
    # If reset time has passed today, use tomorrow's reset time
    if [ "$now_timestamp" -gt "$today_reset" ]; then
        resume_timestamp=$(LC_ALL=C date -d "tomorrow $reset_time_str" +%s)
    else
