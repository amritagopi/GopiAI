...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # Настройки окна
        self.setWindowTitle(tr("browser_agent_demo.title", "GopiAI - Демонстрация Браузерного Агента"))
        self.setMinimumSize(1200, 800)

        # Инициализируем менеджер тем
        theme_manager = ThemeManager.instance(QApplication.instance())
        ...
        # Заголовок чата
        self.chat_title = QLabel(tr("browser_agent_demo.chat_title", "Браузерный Агент"))
        ...
        self.chat_history.setStyleSheet(f"""
            QTextEdit {{
                ...
                padding: 8px;
                font-size: 14px;
                color: {theme_manager.get_color('foreground')};
            }}
        """)
        ...
        self.input_field.setPlaceholderText(tr("browser_agent_demo.input_placeholder", "Введите запрос для агента..."))
        ...
        # Кнопка отправки
        self.send_button = QPushButton(tr("browser_agent_demo.send", "Отправить"))
        self.send_button.setStyleSheet(f"""
            ...
            QPushButton:pressed {{
                background-color: {theme_manager.get_color('button_pressed')};
            }}
        """)
        self.input_layout.addWidget(self.send_button)

        # Кнопка остановки
        self.stop_button = QPushButton(tr("browser_agent_demo.stop", "Остановить"))
        self.stop_button.setEnabled(False)
        self.stop_button.setStyleSheet(f"""
            QPushButton {{
                background-color: {theme_manager.get_color('error')};
                color: {theme_manager.get_color('foreground')};
                border: none;
                border-radius: 4px;
                padding: 8px 16px;
        ...
        self.status_label = QLabel(tr("browser_agent_demo.ready", "Готов к работе"))
        ...
        self.browser_widget.add_browser_tab("https://www.google.com")
    ...
    def on_agent_error(self, error):
        """Обрабатывает ошибку от агента."""
        self.chat_history.append(f"<b>{tr('browser_agent_demo.error', 'Ошибка')}:</b> {error}")
        self.chat_history.append("")  # Пустая строка для разделения сообщений
        self.chat_history.ensureCursorVisible()
        self.status_label.setText(tr("browser_agent_demo.error", "Ошибка"))

    @Slot(bool)
    def on_agent_thinking(self, is_thinking):
        """Обрабатывает состояние размышления агента."""
        if is_thinking:
            self.status_label.setText(tr("browser_agent_demo.processing", "Агент обрабатывает запрос..."))
            self.input_field.setEnabled(False)
            self.send_button.setEnabled(False)
            self.stop_button.setEnabled(True)
        else:
            self.status_label.setText(tr("browser_agent_demo.ready", "Готов к работе"))
