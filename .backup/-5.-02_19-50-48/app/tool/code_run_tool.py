...
class CodeRunTool(BaseTool):
    ...
    async def _run_python_code(self, code: str) -> CLIResult:
        ...
        try:
            # В реальном приложении здесь будет запуск в песочнице
            # Сейчас просто компилируем код, чтобы убедиться, что он синтаксически корректен
            compile(code, '<string>', 'exec')

            # Здесь должен быть настоящий запуск кода
            # Возвращаем заглушку
            return CLIResult(output="Код успешно запущен (симуляция)")
        except Exception as e:
