        ...
        try:
            ...
            try:
                ...
                if not isinstance(result, dict):
                    self.notify_ui(f"Неожиданный результат: {result}")
                    return ToolResult(error=f"Неожиданный результат: {result}")

                if not result.get("success", False):
                    self.notify_ui(f"Ошибка при создании скриншота: {result.get('error', 'неизвестная ошибка')}")
                ...
                if not image_data:
                    self.notify_ui("Не удалось получить данные изображения")
                ...
                self.notify_ui(f"Скриншот сохранен в {full_path}")
                return ToolResult(output=f"Скриншот сохранен в {full_path}")

            except asyncio.TimeoutError:
                self.notify_ui("Таймаут при создании скриншота")
                return ToolResult(error="Таймаут при создании скриншота")
            except Exception as e:
                self.notify_ui(f"Ошибка при обработке скриншота: {str(e)}")
                return ToolResult(error=f"Ошибка при обработке скриншота: {str(e)}")

        except Exception as e:
            self.notify_ui(f"Ошибка при создании скриншота: {str(e)}")
