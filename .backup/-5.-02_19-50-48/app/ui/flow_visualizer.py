...
def show_flow_visualizer_dialog(flow, parent=None):
    ...
    Args:
        flow: Поток (Flow) для визуализации
        parent: Родительский виджет

    Returns:
        int: Результат выполнения диалога (принят/отклонен)
    """
    if flow is None:
        # Если поток не предоставлен, показываем сообщение об ошибке
        QMessageBox.warning(
            parent,
