...
class MainWindow(QMainWindow):
    ...
    def _setup_ui(self):
        ...
        try:
            ...
            # Настраиваем меню (должно быть перед созданием панелей инструментов!)
            self._setup_menus()
            ...
            # Проверка и исправление ошибок в компонентах
            self._validate_ui_components()

            # Применяем ограничения для доков
            self._apply_dock_constraints()

            # Подключаем сигналы
            self._connect_ui_signals()

            # Обновляем переводы всех компонентов
            ...
            self._on_language_changed_event(current_language)

            # Применяем начальный макет интерфейса
            self._apply_initial_layout()

            # Запускаем обработку событий для корректной отрисовки
    ...
    def _create_docks(self):
        ...
        # Метод отвечает за создание и настройку всех док-виджетов в приложении
        # Изменение логики может привести к поломке UI и нарушению работы приложения
        #
        # Макет интерфейса должен соответствовать следующим принципам:
        # 1. Файловый проводник - всегда закреплен слева, не может быть отсоединен
        # 2. Чат с ИИ - всегда закреплен справа, не может быть отсоединен
        # 3. Терминал - закреплен внизу, может изменять размер, имеет вкладки
        # 4. Текстовый редактор - основной центральный элемент, имеет вкладки
        #
        # Окна имеют логичное поведение:
        # - Все окна кроме файлового проводника и чата могут быть плавающими
        # - Окна могут быть прикреплены сверху или снизу
        # - Вкладки всегда имеют согласованное поведение
        ############################################################################
        try:
            # --- Левая панель: Проводник проекта ---
            self.project_explorer_dock = QDockWidget(
                self._translate("dock.project_explorer", "Проводник проекта"), self
            )
            ...
            self.project_explorer_dock.setObjectName(
                "ProjectExplorerDock"
            )  # Имя для сохранения настроек
            ...
            # Применяем кастомный заголовок без кнопок к постоянно закрепленным окнам
            apply_custom_title_bar(self.project_explorer_dock, is_docked_permanent=True)

            # Подключаем сигнал двойного клика на файл
            self.project_explorer.file_double_clicked.connect(
                self._on_file_double_clicked
            )

            # --- Создаем чат-виджет и док для него ---
            self.chat_dock = QDockWidget(self._translate("dock.chat", "ИИ-чат"), self)
            self.chat_dock.setFeatures(QDockWidget.DockWidgetFeature.DockWidgetMovable)
            ...
            # Подключаем сигнал изменения видимости
            self.chat_dock.visibilityChanged.connect(self._on_dock_visibility_changed)
            ...
            self.terminal_dock = QDockWidget(
                self._translate("dock.terminal", "Терминал"), self
            )
            ...
            self.terminal_dock.setObjectName(
                "TerminalDock"
            )  # Имя для сохранения настроек
    ...
    def _validate_ui_components(self):
        ...
        for dock_name in [
            ...
            if not hasattr(self, dock_name) or getattr(self, dock_name) is None:
                logger.warning(
                    f"Док-виджет {dock_name} отсутствует, восстанавливаем доки"
                )
                self._create_docks()
                break
        ...
        try:
            ...
            if os.path.exists(style_path) or force_reload:
                with open(style_path, "r", encoding="utf-8") as f:
                    ...
                    is_dark_theme = (
                        "dark" in ThemeManager.instance().get_current_theme()
                    )
    ...
    def _create_actions(self):
        ...
        self.open_file_action.setStatusTip(
            self._translate("menu.open_file.tooltip", "")
        )
        ...
        self.select_all_action.setStatusTip(
            self._translate("menu.select_all.tooltip", "")
        )
        ...
        self.toggle_chat_action.setStatusTip(
            self._translate("dock.chat.toggle.tooltip", "")
        )
        ...
        self.toggle_browser_action.setStatusTip(
            self._translate("dock.browser.toggle.tooltip", "")
        )
        ...
        self.reset_layout_action.setStatusTip(
            self._translate("menu.reset_layout.tooltip", "")
        )
        ...
        self.open_url_action.setStatusTip(
            self._translate("menu.tools.open_url.tooltip", "")
        )
        ...
        self.configure_agent_action.setStatusTip(
            self._translate("menu.configure_agent.tooltip", "")
        )
        ...
        self.view_flow_action.setStatusTip(
            self._translate("menu.view_flow.tooltip", "")
        )
        ...
        self.preferences_action.setStatusTip(
            self._translate("menu.preferences.tooltip", "")
        )
        ...
        self.documentation_action.setStatusTip(
            self._translate("menu.documentation.tooltip", "")
        )
        ...
        # Диагностика UI (добавим в меню Tools)
        self.run_diagnostics_action = QAction(
            get_icon("debug"),
            tr("menu.tools.run_diagnostics", "Run UI Diagnostics"),
            self,
        )
        self.run_diagnostics_action.setStatusTip(
            tr("menu.tools.run_diagnostics.tooltip", "Test and diagnose UI components")
        )
        self.run_diagnostics_action.triggered.connect(self._run_ui_diagnostics)
    ...
    def _open_coding_agent(self):
        ...
        try:
            ...
            self.statusBar().showMessage(
                tr("main.coding_agent_opened", "Coding Agent opened"), 3000
            )
        except Exception as e:
            ...
            QMessageBox.critical(
                ...
                tr(
                    "dialogs.coding_agent_error", "Error opening Coding Agent: {error}"
                ).format(error=str(e)),
            )
    ...
    def _open_browsing_agent(self):
        ...
        try:
            ...
            # Добавляем информацию в статусную строку
            self.statusBar().showMessage(
                tr("main.browsing_agent_opened", "Browsing Agent opened"), 3000
            )
        except Exception as e:
            ...
            QMessageBox.critical(
                ...
                tr("dialogs.error", "Error"),
                tr(
                    "dialogs.browsing_agent_error",
                    "Error opening Browsing Agent: {error}",
                ).format(error=str(e)),
            )
    ...
    def _setup_menus(self):
        ...
        try:
            ...
            # Используем уже существующее меню Tools вместо создания нового
            self.tools_menu = self.menu_manager.tools_menu

            # Копируем действия в атрибуты для доступа из методов
            self.new_file_action = self.menu_manager.new_file_action
            self.open_file_action = self.menu_manager.open_file_action
            self.save_file_action = self.menu_manager.save_file_action
            self.save_as_action = self.menu_manager.save_as_action
            self.exit_action = self.menu_manager.exit_action

            self.cut_action = self.menu_manager.cut_action
            self.copy_action = self.menu_manager.copy_action
            self.paste_action = self.menu_manager.paste_action
            self.undo_action = self.menu_manager.undo_action
            self.redo_action = self.menu_manager.redo_action
            self.select_all_action = self.menu_manager.select_all_action
            self.emoji_action = self.menu_manager.emoji_action

            self.toggle_terminal_action = self.menu_manager.toggle_terminal_action
            self.toggle_project_explorer_action = (
                self.menu_manager.toggle_project_explorer_action
            )
            self.toggle_chat_action = self.menu_manager.toggle_chat_action
            self.toggle_browser_action = self.menu_manager.toggle_browser_action
            self.reset_layout_action = self.menu_manager.reset_layout_action

            self.open_url_action = self.menu_manager.open_url_action
            self.configure_agent_action = self.menu_manager.configure_agent_action
            self.view_flow_action = self.menu_manager.view_flow_action
            self.preferences_action = self.menu_manager.preferences_action

            # Копируем действие для диагностики UI
            if hasattr(self.menu_manager, "run_ui_diagnostics_action"):
                self.run_diagnostics_action = (
                    self.menu_manager.run_ui_diagnostics_action
                )
            else:
                # Создаем действие, если оно не существует в menu_manager
                from app.ui.i18n.translator import tr
                ...
                self.run_diagnostics_action.setStatusTip(
                    tr(
                        "menu.tools.run_diagnostics.tooltip",
                        "Test and diagnose UI components",
                    )
                )
                self.run_diagnostics_action.triggered.connect(self._run_ui_diagnostics)

            self.about_action = self.menu_manager.about_action
            self.documentation_action = self.menu_manager.documentation_action

            # Подключаем сигналы
            self.menu_manager.theme_changed.connect(self._on_theme_changed)
            self.menu_manager.language_changed.connect(self._on_language_changed)

            # Добавляем диагностику в меню Tools, если она еще не добавлена в меню
            if hasattr(self, "run_diagnostics_action") and self.tools_menu:
                # Проверяем, не добавлено ли уже это действие
                already_added = False
                for action in self.tools_menu.actions():
                    if action == self.run_diagnostics_action:
                        already_added = True
                        break
            ...
            logger.info("Меню успешно настроено")
        except Exception as e:
    ...
    def _update_view_menu(self):
        ...
        if hasattr(self, "toggle_project_explorer_action"):
            self.toggle_project_explorer_action.setChecked(
                ...
                and self.project_explorer_dock.isVisible()
            )
    ...
    def _connect_ui_signals(self):
        ...
        if hasattr(self, "central_tabs"):
            ...
            self.central_tabs.customContextMenuRequested.connect(
        ...
        if hasattr(self, "project_explorer"):
            self.project_explorer.tree_view.doubleClicked.connect(
                self._on_project_tree_double_clicked
            )
        ...
        if hasattr(self, "chat_widget"):
            ...
            if hasattr(self, "terminal_widget") and hasattr(
                self.terminal_widget, "command_executed"
            ):
                self.terminal_widget.command_executed.connect(
                    lambda cmd, out: self.chat_widget.log_event(
    ...
    def _on_project_tree_double_clicked(self, index):
        ...
        try:
            ...
            if file_info.isDir():
                ...
                self.project_explorer.tree_view.setExpanded(
                    index, not self.project_explorer.tree_view.isExpanded(index)
                )
            else:
        ...
        try:
            ...
            if file_info.isDir():
                ...
                open_in_explorer = QAction(
                    self._translate(
                        "context_menu.open_in_explorer", "Открыть в проводнике"
                    ),
                    self,
                )
                ...
                # Опции для файлов
                open_file = QAction(
                    self._translate("context_menu.open", "Открыть"), self
                )
                ...
                show_in_explorer = QAction(
                    self._translate(
                        "context_menu.show_in_explorer", "Показать в проводнике"
                    ),
                    self,
                )
                show_in_explorer.triggered.connect(
                    lambda: self._open_in_explorer(os.path.dirname(file_path))
                )
    ...
    def _update_ui_translations(self):
        ...
        if (
            hasattr(self, "_is_updating_translations")
            and self._is_updating_translations
        ):
            return
        ...
        try:
            ...
            if hasattr(self, "new_file_action"):
                ...
                self.new_file_action.setStatusTip(
                    self._translate("menu.new.tooltip", "")
                )
            ...
            if hasattr(self, "paste_action"):
                ...
                self.paste_action.setStatusTip(
                    self._translate("menu.paste.tooltip", "")
                )

            if hasattr(self, "emoji_action"):
                self.emoji_action.setText(self._translate("menu.insert_emoji", ""))
                self.emoji_action.setStatusTip(
                    self._translate("menu.insert_emoji.tooltip", "")
                )
            ...
            if hasattr(self, "toggle_terminal_action"):
                self.toggle_terminal_action.setText(
                    self._translate("dock.terminal.toggle", "")
                )
            ...
            if hasattr(self, "configure_agent_action"):
                self.configure_agent_action.setText(
                    self._translate("menu.configure_agent", "")
                )
                self.configure_agent_action.setStatusTip(
                    self._translate("menu.configure_agent.tooltip", "")
                )
            ...
            if hasattr(self, "documentation_action"):
                self.documentation_action.setText(
                    self._translate("menu.documentation", "")
                )
            ...
            if hasattr(self, "project_explorer_dock"):
                self.project_explorer_dock.setWindowTitle(
                    self._translate("dock.project_explorer", "")
                )
    ...
    def _setup_central_widget(self):
        ...
        self.central_tabs.setContextMenuPolicy(
        ...
        self.central_tabs.addTab(
            self.code_editor, self._translate("code.new_file", "new_file.py")
        )
    ...
    def _show_tab_context_menu(self, pos):
        ...
        close_all_action = QAction(
            self._translate("dialogs.close_all", "Close All Tabs"), self
        )
        ...
        close_others_action = QAction(
            self._translate("dialogs.close_others", "Close Other Tabs"), self
        )
    ...
    def _close_other_tabs(self, keep_index):
        ...
        indices_to_close = [
            i for i in range(self.central_tabs.count()) if i != keep_index
        ]
    ...
    def _update_themes_menu(self):
        ...
        if hasattr(self, "theme_menu"):
            ...
            for theme in themes:
                theme_display_name = ThemeManager.instance().get_theme_display_name(
                    theme
                )
                ...
                theme_action.triggered.connect(
                    lambda checked, t=theme: self._on_theme_changed(t)
                )
            ...
            customize_action = QAction(
                tr("menu.theme.customize", "Customize Theme..."), self
            )
        ...
        try:
            ...
            for lang in language_options:
                ...
                lang_action.triggered.connect(
                    lambda checked, lc=lang["code"]: self._on_language_changed(lc)
                )
    ...
    def _connect_theme_language_signals(self):
        """Подключает сигналы для работы с темами и языками."""
        try:
            # Подключаем сигналы смены темы
            theme_manager = ThemeManager.instance()

            # Отключаем существующие подключения для избежания дублирования
            try:
                theme_manager.themeChanged.disconnect(self._on_theme_changed_event)
            except Exception:
                logger.debug("Нет существующих подключений к сигналу themeChanged")

            # Подключаем сигнал смены темы
            theme_manager.themeChanged.connect(self._on_theme_changed_event)
            logger.debug(f"Сигнал themeChanged подключен")

            # Подключаем сигналы смены языка
            from app.ui.i18n.translator import JsonTranslationManager
            ...
            # Отключаем существующие подключения для избежания дублирования
            try:
                translator.languageChanged.disconnect(self._on_language_changed_event)
            except Exception:
                logger.debug(
                    "Нет существующих подключений к сигналу languageChanged для _on_language_changed_event"
                )

            # Прямое подключение к главному окну
            translator.languageChanged.connect(self._on_language_changed_event)
            logger.debug(
                f"Сигнал languageChanged подключен к _on_language_changed_event"
            )

            # Подключение к менеджеру меню (если он существует)
            if hasattr(self, "menu_manager"):
                try:
                    ...
                    logger.debug(
                        "Нет существующих подключений к сигналу languageChanged для menu_manager.update_translations"
                    )
                ...
                logger.debug(
                    f"Сигнал languageChanged подключен к menu_manager.update_translations"
                )

            logger.info(f"Сигналы тем и языков успешно подключены")
        except Exception as e:
            logger.error(f"Ошибка при подключении сигналов тем и языков: {str(e)}")
    ...
    def _on_language_changed(self, language_code):
        ...
        try:
            ...
            logger.debug(
                f"Переключение языка на {language_code} через _on_language_changed"
            )
            # Переключаем язык - это вызовет сигнал languageChanged
    ...
    def _retranslate_all_menus(self):
        """Обновляет тексты всех меню согласно текущему языку."""
        try:
            # Обновляем меню через менеджер меню
            if hasattr(self, "menu_manager"):
                # Если у менеджера меню есть метод update_translations, используем его
                if hasattr(self.menu_manager, "update_translations"):
                    self.menu_manager.update_translations()
                # Иначе обновляем отдельные меню
                else:
                    if hasattr(self.menu_manager, "update_language_menu"):
                        self.menu_manager.update_language_menu()
                    if hasattr(self.menu_manager, "update_themes_menu"):
                        self.menu_manager.update_themes_menu()
        except Exception as e:
            import logging

            logger = logging.getLogger(__name__)
            logger.error(f"Ошибка при обновлении меню: {str(e)}", exc_info=True)

    def _update_dock_titles(self):
        """Обновляет заголовки всех док-виджетов при смене языка."""
        try:
            from app.ui.i18n.translator import tr
            ...
            if hasattr(self, "docks"):
                for dock_name, dock in self.docks.items():
                    if dock_name == "chat":
                        dock.setWindowTitle(tr("dock.chat", "Чат"))
                    elif dock_name == "terminal":
                        dock.setWindowTitle(tr("dock.terminal", "Терминал"))
                    elif dock_name == "browser":
                        dock.setWindowTitle(tr("dock.browser", "Веб-браузер"))
                    elif dock_name == "project_explorer":
                        dock.setWindowTitle(
                            tr("dock.project_explorer", "Проводник проекта")
                        )
                    elif dock_name == "tools":
                        dock.setWindowTitle(tr("dock.tools", "Инструменты"))
                    elif dock_name == "output":
                        dock.setWindowTitle(tr("dock.output", "Вывод"))
        except Exception as e:
            import logging

            logger = logging.getLogger(__name__)
            logger.error(
                f"Ошибка при обновлении заголовков доков: {str(e)}", exc_info=True
            )
    ...
    def _create_toolbars(self):
        ...
        self.main_toolbar = self.addToolBar(
            self._translate("toolbar.main", "Main Toolbar")
        )
        ...
        # Добавляем действия на панель инструментов, если они определены в menu_manager
        if hasattr(self, "new_file_action"):
    ...
    def _update_custom_title_bars(self):
        ...
        try:
            ...
            if hasattr(self, "project_explorer_dock"):
                apply_custom_title_bar(
                    self.project_explorer_dock, is_docked_permanent=True
                )
    ...
    def _fix_duplicate_docks(self):
        """Проверяет и исправляет дублирование доков."""
        try:
            # Получаем все доки в окне
            docks = self.findChildren(QDockWidget)

            # Создаем словарь для отслеживания доков по их objectName
            dock_names = {}
    ...
    def _apply_initial_layout(self):
        ...
        # Проверяем наличие дубликатов доков и фиксируем их
        self._fix_duplicate_docks()

        # Отображаем только один набор доков
        if hasattr(self, "project_explorer_dock"):
        ...
        # Обновляем состояние дейсвтий в меню
        self._update_view_menu()
    ...
    def retranslateUi(self):
        ...
        if hasattr(self, "new_file_action"):
            ...
            self.new_file_action.setStatusTip(
                tr("menu.new.tooltip", "Create a new file")
            )
        ...
        if hasattr(self, "exit_action"):
            ...
            self.exit_action.setStatusTip(
                tr("menu.exit.tooltip", "Exit the application")
            )
        ...
        try:
            ...
            if result == QDialog.Accepted:
                ...
                QMessageBox.information(
                    ...
                    tr(
                        "main_window.theme_settings_success",
                        "Theme and language settings successfully applied.",
                    ),
                )
        except Exception as e:
    ...
    def _new_file(self):
        ...
        try:
            ...
            # Добавляем вкладку с правильным названием согласно текущему языку
            tab_name = self._tab_name_from_path(new_file_name)
            index = self.central_tabs.addTab(editor, tab_name)
            self.central_tabs.setCurrentIndex(index)
    ...
    def _save_file(self):
        ...
        try:
            ...
            is_new_file = (
                editor.file_path == tr("code.new_file", "new_file.py")
                or editor.file_path == "new_file.py"
                or editor.file_path == "новый_файл.py"
            )

            # Если это новый файл, вызываем save_as
            if not editor.file_path or is_new_file:
            ...
            self.central_tabs.setTabText(
                current_index, os.path.basename(editor.file_path)
            )
    ...
    def _save_file_as(self):
        ...
        try:
            ...
            if editor.file_path and editor.file_path != tr(
                "code.new_file", "new_file.py"
            ):
    ...
    def _open_url_in_browser(self):
        ...
        try:
            ...
            QMessageBox.warning(
                self, tr("dialogs.error.title", "Error"), f"Error opening URL: {str(e)}"
            )
        ...
        try:
            ...
            if not hasattr(self, "agent") or self.agent is None:
                QMessageBox.warning(
                    ...
                    tr(
                        "agent.not_initialized_message",
                        "Please initialize the agent first.",
                    ),
                )
                return
            ...
            if flow is None:
                QMessageBox.information(
                    ...
                    tr(
                        "flow.no_flow_message",
                        "The current agent does not have an available flow to visualize.",
                    ),
                )
                return
    ...
    def _run_ui_diagnostics(self):
        """Запускает инструмент диагностики пользовательского интерфейса."""
        from app.ui.debug_ui import run_ui_diagnostics

        run_ui_diagnostics(self)

    def _get_current_flow(self):
        """Возвращает текущий поток для визуализации."""
        # Implement this method to return the current flow for visualization
        pass
    ...
    def _on_about(self):
        ...
        try:
            ...
            about_text = tr(
                "about.description", "An AI assistant with advanced capabilities."
            )
    ...
    def _on_documentation(self):
        ...
        try:
            ...
            doc_text = tr(
                "documentation.not_implemented", "Documentation is not implemented yet."
            )
    ...
    def show_reasoning_agent(self):
        ...
        try:
            ...
            QMessageBox.warning(
                ...
                self.tr(
                    "menu.reasoning_agent.error.message",
    ...
    def show_reasoning_settings(self):
        ...
        try:
            ...
            QMessageBox.warning(
                ...
                self.tr(
                    "menu.reasoning_settings.error.message",
                    "Failed to open Reasoning Settings dialog. Error: {0}",
                ).format(str(e)),
            )
    ...
    def reset_dock_layout(self):
        """Сбрасывает расположение панелей на стандартное."""
        try:
            # Скрываем все панели
            if hasattr(self, "terminal_dock"):
                self.terminal_dock.hide()
            ...
            # Закрепляем проводник проекта слева
            if hasattr(self, "project_explorer_dock"):
                self.project_explorer_dock.setVisible(True)
                self.addDockWidget(Qt.LeftDockWidgetArea, self.project_explorer_dock)

            # Закрепляем чат справа
            if hasattr(self, "chat_dock"):
                self.chat_dock.setVisible(True)
                self.addDockWidget(Qt.RightDockWidgetArea, self.chat_dock)
            ...
            # Обновляем статус
            self.statusBar().showMessage(
                tr("status.layout_reset", "Layout reset to default"), 3000
            )
            logger.info("Dock layout has been reset")
        except Exception as e:
            logger.error(f"Error resetting dock layout: {e}")

    def _tab_name_from_path(self, file_path):
        """Формирует имя вкладки из пути к файлу."""
        from app.ui.i18n.translator import tr

        # Если это новый файл без пути, используем перевод
        if (
            file_path == tr("code.new_file", "new_file.py")
            or file_path == "new_file.py"
            or file_path == "новый_файл.py"
        ):
            return tr("code.new_file", "new_file.py")

        # Для обычных файлов возвращаем только имя файла без пути
        return os.path.basename(file_path)

    def _update_tab_titles(self):
        """Обновляет заголовки всех вкладок при смене языка."""
        try:
            tab_count = self.central_tabs.count()
            for i in range(tab_count):
                widget = self.central_tabs.widget(i)
                if hasattr(widget, "file_path"):
                    tab_name = self._tab_name_from_path(widget.file_path)
                    self.central_tabs.setTabText(i, tab_name)
        except Exception as e:
            logger.error(f"Ошибка при обновлении заголовков вкладок: {str(e)}")
