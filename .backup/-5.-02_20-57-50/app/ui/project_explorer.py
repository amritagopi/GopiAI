...
from .icon_manager import get_icon, IconManager
...
class ProjectExplorer(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.refresh_label.setPixmap(get_icon("refresh").pixmap(16, 16))
    ...
    def _initialize_paths(self):
        ...
        self.path_combo.addItem(get_icon("folder"), "Текущая директория", current_dir)
        self.path_combo.addItem(get_icon("home_folder"), "Домашняя директория", QDir.homePath())

        # Добавляем корневые диски (для Windows)
        if sys.platform == "win32":
            for drive in QDir.drives():
                drive_path = drive.absolutePath()
                self.path_combo.addItem(get_icon("drive"), drive_path, drive_path)
    ...
    def _set_current_path(self, path):
        ...
        if not found and os.path.exists(path):
            self.path_combo.addItem(get_icon("folder"), os.path.basename(path), path)
    ...
    def _show_context_menu(self, position):
        ...
        if is_dir:
            ...
            open_action = menu.addAction(get_icon("folder_open"), tr("menu.open", "Открыть"))
            menu.addSeparator()
            new_file_action = menu.addAction(get_icon("new"), tr("menu.new_file", "Новый файл"))
            new_dir_action = menu.addAction(get_icon("new_folder"), tr("menu.new_folder", "Новая папка"))
        else:
            # Действия для файла
            open_action = menu.addAction(get_icon("edit"), tr("menu.open", "Открыть"))

        menu.addSeparator()
        rename_action = menu.addAction(get_icon("rename"), tr("menu.rename", "Переименовать"))
        delete_action = menu.addAction(get_icon("delete"), tr("menu.delete", "Удалить"))
