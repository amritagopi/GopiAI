...
class JsonTranslationManager(QObject):
    ...
    def load_translations(self):
        ...
        # Сначала проверим наличие файлов переводов в директории i18n
        ...
        # Затем проверим наличие файлов в директории themes
        ...
        # Загружаем английские переводы из themes (базовый уровень)
        if os.path.exists(en_path_themes):
            try:
                with open(en_path_themes, 'r', encoding='utf-8') as f:
                    themes_translations = json.load(f)
                    self.translations["en_US"].update(themes_translations)
                    logger.info(f"Загружены английские переводы (themes): {len(themes_translations)} записей")
            except Exception as e:
                logger.error(f"Ошибка при загрузке английских переводов (themes): {str(e)}")

        # Загружаем русские переводы из themes (базовый уровень)
        if os.path.exists(ru_path_themes):
            try:
                with open(ru_path_themes, 'r', encoding='utf-8') as f:
                    themes_translations = json.load(f)
                    self.translations["ru_RU"].update(themes_translations)
                    logger.info(f"Загружены русские переводы (themes): {len(themes_translations)} записей")
            except Exception as e:
                logger.error(f"Ошибка при загрузке русских переводов (themes): {str(e)}")

        # Загружаем английские переводы из i18n (приоритетный уровень)
        if os.path.exists(en_path_i18n):
            try:
                with open(en_path_i18n, 'r', encoding='utf-8') as f:
                    i18n_translations = json.load(f)
                    self.translations["en_US"].update(i18n_translations)  # Переопределит любые дублированные ключи
                    logger.info(f"Загружены английские переводы (i18n): {len(i18n_translations)} записей")
            except Exception as e:
                logger.error(f"Ошибка при загрузке английских переводов (i18n): {str(e)}")

        # Загружаем русские переводы из i18n (приоритетный уровень)
        if os.path.exists(ru_path_i18n):
            try:
                with open(ru_path_i18n, 'r', encoding='utf-8') as f:
                    i18n_translations = json.load(f)
                    self.translations["ru_RU"].update(i18n_translations)  # Переопределит любые дублированные ключи
                    logger.info(f"Загружены русские переводы (i18n): {len(i18n_translations)} записей")
            except Exception as e:
                logger.error(f"Ошибка при загрузке русских переводов (i18n): {str(e)}")
