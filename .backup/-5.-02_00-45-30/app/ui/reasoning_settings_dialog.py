...
class ReasoningSettingsDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.setWindowTitle(tr("agent.reasoning.settings_title", "Настройки рассуждений"))
    ...
    def _setup_ui(self):
        ...
        reasoning_group = QGroupBox(tr("agent.reasoning.parameters_group", "Параметры рассуждений"))
        ...
        reasoning_layout.addRow(tr("agent.reasoning.depth", "Глубина рассуждений:"), depth_layout)
        ...
        reasoning_layout.addRow(tr("agent.reasoning.strategy", "Стратегия рассуждений:"), self.strategy_combo)
        ...
        security_group = QGroupBox(tr("agent.reasoning.security_group", "Безопасность и мониторинг"))
        security_layout = QFormLayout()

        # Безопасный режим
        self.safe_mode_checkbox = QCheckBox(tr("agent.reasoning.safe_mode", "Включить проверки безопасности"))
        self.safe_mode_checkbox.setToolTip(
            tr("agent.reasoning.safe_mode.tooltip",
        ...
        # Интерактивный режим
        self.interactive_mode_checkbox = QCheckBox(tr("agent.reasoning.interactive_mode", "Включить интерактивный режим"))
        self.interactive_mode_checkbox.setToolTip(
            tr("agent.reasoning.interactive_mode.tooltip",
               "Требует подтверждение для каждого шага выполнения плана")
        )
        security_layout.addRow("", self.interactive_mode_checkbox)

        # Мониторинг
        self.monitoring_checkbox = QCheckBox(tr("agent.reasoning.monitoring", "Включить мониторинг выполнения"))
        self.monitoring_checkbox.setToolTip(
            tr("agent.reasoning.monitoring.tooltip",
               "Отслеживает выполнение плана и показывает прогресс")
        )
        security_layout.addRow("", self.monitoring_checkbox)

        # Подробное логирование
        self.detailed_logging_checkbox = QCheckBox(tr("agent.reasoning.detailed_logging", "Включить подробное логирование"))
        self.detailed_logging_checkbox.setToolTip(
            tr("agent.reasoning.detailed_logging.tooltip",
        ...
        self.timeout_spinbox.setSuffix(tr("common.time.seconds", " сек"))
        ...
        security_layout.addRow(tr("agent.reasoning.timeout", "Таймаут операций:"), timeout_layout)
        ...
        strategy_info_group = QGroupBox(tr("agent.reasoning.strategy_description_group", "Описание стратегии"))
        ...
        self.reset_button = QPushButton(tr("common.reset", "Сбросить"))
        self.save_button = QPushButton(tr("common.save", "Сохранить"))
        self.cancel_button = QPushButton(tr("common.cancel", "Отмена"))
    ...
    def _get_strategy_display_name(self, strategy: ReasoningStrategy) -> str:
        """Возвращает читаемое имя для стратегии рассуждения."""
        strategy_names = {
            ReasoningStrategy.CHAIN_OF_THOUGHT: tr("agent.reasoning.strategy.cot", "Цепочка мыслей"),
            ReasoningStrategy.TREE_OF_THOUGHT: tr("agent.reasoning.strategy.tot", "Дерево мыслей"),
            ReasoningStrategy.FIRST_PRINCIPLES: tr("agent.reasoning.strategy.fp", "Первые принципы"),
            ReasoningStrategy.ANALYTICAL: tr("agent.reasoning.strategy.analytical", "Аналитическая"),
            ReasoningStrategy.CREATIVE: tr("agent.reasoning.strategy.creative", "Творческая"),
        }
        return strategy_names.get(strategy, str(strategy.value))
