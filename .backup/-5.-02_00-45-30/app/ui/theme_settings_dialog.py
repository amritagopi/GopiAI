...
class ThemeSettingsDialog(QDialog):
    def __init__(self, parent=None):
        ...
        try:
            ...
            current_theme = self.theme_manager.get_current_visual_theme() # Уже было выше, но для отладки здесь тоже полезно
            logger.debug(f"current_theme in color section is: {current_theme}, type: {type(current_theme)}")

            theme_data = self.theme_manager.themes[current_theme]
            logger.debug(f"theme_data type: {type(theme_data)}")
            logger.debug(f"theme_data keys: {list(theme_data.keys()) if isinstance(theme_data, dict) else 'Not a dict'}")
            ...
            for color_name, color_value in actual_colors.items():
                ...
                logger.debug(f"Checking color_name: {color_name}, color_value: {color_value}")
    ...
    def is_light_color(self, color_hex):
        ...
        if not isinstance(color_hex, str):
            logger.warning(f"is_light_color: получено не строковое значение: {color_hex}, считаем темным.")
            return False # Считаем темным по умолчанию

        processed_color = color_hex.lstrip('#')

        if not (len(processed_color) == 6 and all(c in '0123456789abcdefABCDEF' for c in processed_color)):
            logger.warning(f"is_light_color: неверный формат HEX цвета: {color_hex}, считаем темным.")
            return False # Считаем темным по умолчанию
        ...
        try:
            ...
            logger.error(f"Ошибка преобразования цвета '{color_hex}' в is_light_color: {e}. Считаем темным.")
    ...
    def update_color_buttons(self, theme_type):
        ...
        try:
            if theme_type in self.theme_manager.themes:
                ...
                actual_colors = theme_data.get('colors', {})
                logger.debug(f"Updating color buttons for theme {theme_type}. Found colors: {list(actual_colors.keys()) if isinstance(actual_colors, dict) else 'Not a dict or no colors found'}")

                if not isinstance(actual_colors, dict):
                    logger.warning(f"Colors for theme {theme_type} is not a dictionary or not found. Skipping update.")
                    return
    ...
    def change_color(self, color_name):
        ...
        if color.isValid():
            ...
            if theme_type in self.theme_manager.themes:
                ...
                if 'colors' not in custom_theme_data or not isinstance(custom_theme_data['colors'], dict):
                ...
                if not theme_type.startswith("custom_"):
                    ...
                    custom_theme_name = theme_type # Обновляем существующую кастомную тему

                self.theme_manager.add_custom_theme(custom_theme_name, custom_theme_data)
