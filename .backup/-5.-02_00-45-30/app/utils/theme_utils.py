...
def get_additional_qss_template() -> str:
    ...
    logger.debug(f"tooltip_fg: {tooltip_fg}, tooltip_bg: {tooltip_bg}, border_color: {border_color}")
    ...
    try:
        ...
        if close_icon_path:
            ...
            logger.warning("Иконка закрытия вкладки не найдена ни в одном из ожидаемых мест")
...
def on_theme_changed_event(main_window, theme_name: str):
    ...
    try:
        ...
        if hasattr(main_window, "_update_themes_menu") and callable(main_window._update_themes_menu):
            logger.debug("Вызов main_window._update_themes_menu()")
            main_window._update_themes_menu()
        else:
            logger.warning("Метод _update_themes_menu не найден в main_window или не вызываемый")
