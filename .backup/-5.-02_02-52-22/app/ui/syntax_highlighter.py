...
class PythonHighlighter(QSyntaxHighlighter):
    ...
    def __init__(self, parent=None):
        ...
        # Ключевые слова
        rules += [(r'\b' + w + r'\b', 0, STYLES['keyword']) for w in PythonHighlighter.keywords if w not in ['def', 'class']]
        rules += [(r'\bdef\b', 0, STYLES['defclass']), (r'\bclass\b', 0, STYLES['defclass'])]
        ...
        self.highlighting_rules = [(QRegularExpression(pattern), index, fmt)
    ...
    def match_multiline(self, text, delimiter, in_state, style):
        ...
        while start_index >= 0:
            match = delimiter.match(text, start_index + delimiter.pattern().count('')) # Ищем следующий разделитель
            end_index = match.capturedStart()
            if end_index == -1:
                self.setCurrentBlockState(in_state)
