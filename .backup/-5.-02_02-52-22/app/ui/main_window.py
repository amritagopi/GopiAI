...
class MainWindow(QMainWindow):
    ...
    4. Над терминалом — Текстовый редактор:
       - Закреплен сверху и слева, изменяет размер в обоих направлениях
       - Также использует вкладки, как браузер
       - Текстовые вкладки имеют один цвет, а другие типы вкладок — другие цвета, должны быть
         визуально различимы
    ...
    def __init__(self):
        ...
        # Устанавливаем ограничения на перемещение доков согласно требованиям дизайна
    ...
    def _apply_dock_constraints(self):
        """
        Устанавливает ограничения на перемещение и поведение доков
        в соответствии с требованиями дизайна.
        """
        # Проверяем наличие необходимых доков
        if hasattr(self, 'project_explorer_dock'):
            # Файловый проводник: только слева, нельзя отсоединять
        ...
        if hasattr(self, 'chat_dock'):
            # ИИ-чат: только справа, нельзя отсоединять
        ...
        if hasattr(self, 'terminal_dock'):
            # Терминал: внизу, можно менять размер, но нельзя отсоединять
        ...
        if hasattr(self, 'tools_dock'):
            # Инструменты: могут быть сверху или снизу, можно плавающие
            self.tools_dock.setFeatures(
                QDockWidget.DockWidgetFeature.DockWidgetMovable |
                QDockWidget.DockWidgetFeature.DockWidgetFloatable
            )
            self.tools_dock.setAllowedAreas(Qt.TopDockWidgetArea | Qt.BottomDockWidgetArea)

        if hasattr(self, 'output_dock'):
            # Вывод: могут быть сверху или снизу, можно плавающие
            self.output_dock.setFeatures(
                QDockWidget.DockWidgetFeature.DockWidgetMovable |
                QDockWidget.DockWidgetFeature.DockWidgetFloatable
            )
            self.output_dock.setAllowedAreas(Qt.TopDockWidgetArea | Qt.BottomDockWidgetArea)

        if hasattr(self, 'browser_dock'):
            # Браузер: может быть справа или снизу, может быть плавающим
            self.browser_dock.setFeatures(
                QDockWidget.DockWidgetFeature.DockWidgetMovable |
                QDockWidget.DockWidgetFeature.DockWidgetFloatable
            )
            self.browser_dock.setAllowedAreas(Qt.RightDockWidgetArea | Qt.BottomDockWidgetArea)

    def _setup_ui(self):
        """Настраивает пользовательский интерфейс."""
        ############################################################################
        # !!! КРИТИЧЕСКИ ВАЖНО !!! НЕ ИЗМЕНЯТЬ ЭТОТ МЕТОД БЕЗ КРАЙНЕЙ НЕОБХОДИМОСТИ!
        # Метод отвечает за настройку всего интерфейса приложения
        # Изменение логики и порядка инициализации может привести к серьезным сбоям
        # Очередность вызовов методов критически важна!
    ...
    def _create_menus(self):
        ...
        # Добавляем действие для терминала
        self.terminal_action = QAction(
            get_icon("terminal"),
            self._translate("menu.tools.terminal", "Терминал"),
            self
        )
        self.terminal_action.setStatusTip(self._translate("menu.tools.terminal.tooltip", "Открыть встроенный терминал"))
        self.terminal_action.triggered.connect(lambda: self._toggle_terminal(True))
        self.tools_menu.addAction(self.terminal_action)

        # Добавляем действие для веб-браузера
        self.browser_action = QAction(
            get_icon("browser"),
            self._translate("menu.tools.browser", "Веб-браузер"),
            self
        )
        self.browser_action.setStatusTip(self._translate("menu.tools.browser.tooltip", "Открыть встроенный веб-браузер"))
        self.browser_action.triggered.connect(lambda: self._toggle_browser(True))
        self.tools_menu.addAction(self.browser_action)

        # Добавляем действие для открытия URL в браузере
        self.open_url_action = QAction(
            get_icon("link"),
            self._translate("menu.tools.open_url", "Открыть URL..."),
            self
        )
        self.open_url_action.setStatusTip(self._translate("menu.tools.open_url.tooltip", "Открыть URL в браузере"))
    ...
    def _create_docks(self):
        ...
        self.project_explorer_dock.visibilityChanged.connect(self._on_dock_visibility_changed)
        ...
        self.project_explorer.file_double_clicked.connect(self._on_file_double_clicked)
        ...
        self.tools_dock = QDockWidget(self._translate("dock.tools", "Инструменты"), self)
        self.tools_dock.setFeatures(
            QDockWidget.DockWidgetFeature.DockWidgetMovable |
            QDockWidget.DockWidgetFeature.DockWidgetFloatable
        )
        self.tools_dock.setAllowedAreas(Qt.BottomDockWidgetArea)
        self.tools_dock.setMinimumHeight(100)
        self.tools_dock.setObjectName("ToolsDock") # Имя для сохранения настроек

        # Подключаем сигнал изменения видимости
        self.tools_dock.visibilityChanged.connect(self._on_dock_visibility_changed)

        # Создаем виджет инструментов
        ...
        # Применяем кастомный заголовок с нужными кнопками в зависимости от состояния
        apply_custom_title_bar(self.tools_dock, is_docked_permanent=False)

        # --- Нижняя панель: Вывод ---
        self.output_dock = QDockWidget(self._translate("dock.output", "Вывод"), self)
        self.output_dock.setFeatures(
            QDockWidget.DockWidgetFeature.DockWidgetMovable |
            QDockWidget.DockWidgetFeature.DockWidgetFloatable
        )
        self.output_dock.setAllowedAreas(Qt.BottomDockWidgetArea)
        self.output_dock.setMinimumHeight(100)
        self.output_dock.setObjectName("OutputDock") # Имя для сохранения настроек

        # Подключаем сигнал изменения видимости
        self.output_dock.visibilityChanged.connect(self._on_dock_visibility_changed)
        ...
        # Применяем кастомный заголовок с нужными кнопками в зависимости от состояния
        apply_custom_title_bar(self.output_dock, is_docked_permanent=False)

        # --- Правая панель: Браузер ---
        self.browser_dock = QDockWidget(self._translate("dock.browser", "Веб-браузер"), self)
        self.browser_dock.setFeatures(
            QDockWidget.DockWidgetFeature.DockWidgetMovable |
            QDockWidget.DockWidgetFeature.DockWidgetFloatable
        )
        self.browser_dock.setAllowedAreas(Qt.RightDockWidgetArea | Qt.BottomDockWidgetArea)
        self.browser_dock.setMinimumWidth(400)
        self.browser_dock.setMinimumHeight(300)
        self.browser_dock.setObjectName("BrowserDock") # Имя для сохранения настроек

        # Подключаем сигнал изменения видимости
        self.browser_dock.visibilityChanged.connect(self._on_dock_visibility_changed)

        # Создаем виджет браузера
        self.browser_widget = MultiBrowserWidget(self)
        ...
        # Скрываем браузер по умолчанию
        self.browser_dock.hide()

        # --- Терминал ---
        self.terminal_dock = QDockWidget(self._translate("dock.terminal", "Терминал"), self)
        self.terminal_dock.setFeatures(
            QDockWidget.DockWidgetFeature.DockWidgetMovable
        )
        self.terminal_dock.setAllowedAreas(Qt.BottomDockWidgetArea)
        ...
        # Подключаем сигнал изменения видимости
        self.terminal_dock.visibilityChanged.connect(self._on_dock_visibility_changed)
        ...
        # --- Табулируем доки в нижней части ---
        self.tabifyDockWidget(self.tools_dock, self.output_dock)
        self.tabifyDockWidget(self.output_dock, self.terminal_dock)
        self.tools_dock.raise_()  # Показываем инструменты поверх вывода
    ...
    def _on_dock_visibility_changed(self, visible):
        """Обрабатывает изменение видимости док-виджетов."""
    ...
    def _update_view_menu(self):
        ...
        if hasattr(self, 'toggle_terminal_action') and hasattr(self, 'terminal_dock'):
            self.toggle_terminal_action.setChecked(self.terminal_dock.isVisible())
        if hasattr(self, 'toggle_project_explorer_action') and hasattr(self, 'project_explorer_dock'):
            self.toggle_project_explorer_action.setChecked(self.project_explorer_dock.isVisible())
        if hasattr(self, 'toggle_chat_action') and hasattr(self, 'chat_dock'):
    ...
    def _connect_ui_signals(self):
        ...
        if hasattr(self, 'project_tree'):
            self.project_tree.doubleClicked.connect(self._on_file_double_clicked)
    ...
    def _apply_initial_layout(self):
        ...
        # Установить минимальные размеры для рабочей области
        self.setMinimumSize(1200, 800)

        # Установить размеры доков
        if hasattr(self, 'project_explorer_dock'):
            # Файловый проводник слева
            self.project_explorer_dock.show()
            self.project_explorer_dock.setMinimumWidth(200)
            self.project_explorer_dock.setMaximumWidth(500)

        if hasattr(self, 'chat_dock'):
            # ИИ-чат справа
            self.chat_dock.show()
            self.chat_dock.setMinimumWidth(250)
            self.chat_dock.setMaximumWidth(600)

        if hasattr(self, 'terminal_dock'):
            # Терминал внизу
            self.terminal_dock.show()  # Показываем терминал по умолчанию
            self.terminal_dock.setMinimumHeight(120)
        ...
        if hasattr(self, 'browser_dock'):
            # Браузер скрыт по умолчанию
            self.browser_dock.hide()

        # Разместить редактор так, чтобы он занимал всё доступное пространство
        if hasattr(self, 'central_tabs'):
            self.central_tabs.setMinimumHeight(300)

        # Размещаем доки в табах внизу
        if hasattr(self, 'tools_dock') and hasattr(self, 'output_dock') and hasattr(self, 'terminal_dock'):
            self.tabifyDockWidget(self.tools_dock, self.output_dock)
            self.tabifyDockWidget(self.output_dock, self.terminal_dock)
            self.tools_dock.raise_()  # Показываем инструменты поверх остальных
    ...
    ############################################################################
    # !!! КРИТИЧЕСКИ ВАЖНО !!! НЕ ИЗМЕНЯТЬ ЭТОТ МЕТОД БЕЗ КРАЙНЕЙ НЕОБХОДИМОСТИ!
    # Метод отвечает за принудительное обновление стилей всех виджетов
    # Изменение логики может привести к некорректному отображению интерфейса
    # Особенно важна правильная обработка разных типов виджетов и предотвращение рекурсии
    # Тесно связан с методами _load_styles и _on_theme_changed
    ############################################################################
        ...
        try:
            ...
            for widget in QApplication.allWidgets():
                if widget.isWidgetType():
                    ...
                    # Особая обработка для разных типов виджетов
                    from PySide6.QtWidgets import QHeaderView, QTreeView, QAbstractItemView

                    if isinstance(widget, QAbstractItemView):
                        # Для QAbstractItemView обновляем весь виджет без аргументов
    ...
    ############################################################################
    # !!! КРИТИЧЕСКИ ВАЖНО !!! НЕ ИЗМЕНЯТЬ ЭТОТ МЕТОД БЕЗ КРАЙНЕЙ НЕОБХОДИМОСТИ!
    # Метод отвечает за обработку изменения темы и обновление всего интерфейса
    # Изменение логики может привести к поломке UI и нарушению работы приложения
    # Особенно важны защита от рекурсии и правильный порядок обновления компонентов
    # Тесно связан с ThemeManager.switch_theme и функциями _update_ui_translations
    ############################################################################
    def _on_theme_changed(self, theme_name):
        ...
        self._force_style_reload()
            ...
            # Обновляем кнопки закрытия вкладок
            CloseButtonFixer.apply_to_window(self)

            # Также обновляем все кастомные заголовки доков
    ...
    def _update_custom_title_bars(self):
        """Обновляет кастомные заголовки для всех доков при смене темы."""
        # Обновляем заголовки всех существующих доков
        docks = [
            'project_explorer_dock', 'chat_dock', 'tools_dock',
            'output_dock', 'browser_dock', 'terminal_dock'
        ]

        # Словарь для определения, является ли док постоянно закрепленным
        is_permanent = {
            'project_explorer_dock': True,
            'chat_dock': True,
            'terminal_dock': True,
            'tools_dock': False,
            'output_dock': False,
        ...
        for dock_name in docks:
            if hasattr(self, dock_name):
                ...
                # Определяем, является ли док постоянно закрепленным
                dock_is_permanent = is_permanent.get(dock_name, False)

                # Применяем новый заголовок
                from .dock_title_bar import apply_custom_title_bar
                apply_custom_title_bar(dock, is_docked_permanent=dock_is_permanent)
    ...
    ############################################################################
    # !!! КРИТИЧЕСКИ ВАЖНО !!! НЕ ИЗМЕНЯТЬ ЭТОТ МЕТОД БЕЗ КРАЙНЕЙ НЕОБХОДИМОСТИ!
    # Метод отвечает за обработку выбора языка пользователем и применение изменений
    # Изменение логики может привести к рассинхронизации языка и темы
    # Важен для корректного обновления интерфейса при смене языка
    # Тесно связан с ThemeManager.switch_theme и ThemeManager.switch_language
    ############################################################################
    ...
    def closeEvent(self, event):
        ...
        # Корректно завершаем поток агента, если он существует
        if hasattr(self, 'agent_worker') and self.agent_worker is not None:
            if hasattr(self.agent_worker, 'stop_loop') and callable(self.agent_worker.stop_loop):
                self.agent_worker.stop_loop()

        if hasattr(self, 'agent_thread') and self.agent_thread is not None:
            if self.agent_thread.isRunning():
                print("Stopping agent thread before closing...")
                self.agent_thread.quit()
                # Ждем завершения потока с таймаутом
                if not self.agent_thread.wait(1000):
                    print("Agent thread did not quit in time, terminating...")
    ...
    def _open_url_in_browser(self):
        ...
        url, ok = QInputDialog.getText(
            self,
            self._translate("dialogs.open_url", "Открыть URL"),
            self._translate("dialogs.enter_url", "Введите URL для открытия:"),
            QLineEdit.Normal,
            "https://"
        )

        if ok and url:
            # Показываем браузер, если он скрыт
            if not self.browser_dock.isVisible():
                self.browser_dock.show()

            # Создаем новую вкладку с указанным URL
            if hasattr(self, 'browser_widget'):
                self.browser_widget.add_new_tab(url)

            # Обновляем статус
            self.status_label.setText(self._translate("status.url_opened", "URL открыт") + f": {url}")
