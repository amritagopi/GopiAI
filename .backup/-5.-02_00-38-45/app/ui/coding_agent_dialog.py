...
from PySide6.QtCore import Qt, Signal, Slot, QSize, QPoint, QSettings
...
from app.ui.icon_manager import IconManager # Изменено
...
class CodingAgentDialog(QDialog):
    ...
    def __init__(self, icon_manager: IconManager, parent=None, theme_manager: Optional[ThemeManager] = None): # Добавлен icon_manager
        super().__init__(parent)
        ...
        self.setWindowIcon(self.icon_manager.get_icon("code")) # Используем self.icon_manager
    ...
    def init_ui(self):
        ...
        self.editor_widget = MultiEditorWidget(self.icon_manager, self, self.theme_manager) # Передаем icon_manager
        ...
        self.send_code_button.setIcon(self.icon_manager.get_icon("chat")) # Используем chat, так как send_to_chat может отсутствовать
        ...
        self.check_code_button.setIcon(self.icon_manager.get_icon("debug")) # Используем debug, так как check может отсутствовать
    ...
    def closeEvent(self, event: QCloseEvent):
        ...
        try:
            ...
            settings.setValue("coding_agent_geometry", self.saveGeometry())
            settings.setValue("coding_agent_visible", False)
            logger.info("Сохранены размер и позиция окна Coding Agent")
        except Exception as e:
            logger.error(f"Ошибка сохранения размера и позиции окна Coding Agent: {e}")
