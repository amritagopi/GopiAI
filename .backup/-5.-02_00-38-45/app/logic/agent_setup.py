...
from app.logic.agent_controller import AgentController  # Добавляем импорт
from app.ui.agent_ui_integration import (
    integrate_all_agents,
    connect_editor_chat_signals,
    # Импортируем константы из agent_ui_integration.py для предотвращения циклических импортов
...
def handle_agent_response(main_window, response):
    ...
    if hasattr(main_window, "chat_widget") and main_window.chat_widget:
        if isinstance(response, str):
            ...
            logger.warning(f"Received non-string agent response: {type(response)}, content: {response}")
    else:
...
def setup_agent(main_window):
    ...
    try:
        ...
        component_id = agent_controller.register_component(MAIN_CHAT_COMPONENT_ID)

        # Регистрируем обратные вызовы для обработки событий
        agent_controller.register_callback(
            component_id,
            "message",
            lambda msg: handle_agent_response(main_window, msg)
        )
        ...
        agent_controller.register_callback(
            ...
            lambda thinking: update_agent_status_display(
                main_window,
                "Thinking... 🤔" if thinking else "Ready"
            )
        )
