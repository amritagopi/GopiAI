...
class PreferencesDialog(QDialog):
    ...
    def _load_settings(self):
        ...
        try:
            # Загружаем настройки для языка
            saved_language = self.settings.value("language", "en_US", str)
            language_index = self.language_combo.findData(saved_language)
            if language_index >= 0:
                self.language_combo.setCurrentIndex(language_index)
                self.selected_language = saved_language

            # Загружаем настройки для темы
            saved_theme = self.settings.value("theme", "light", str)
            theme_index = self.theme_combo.findData(saved_theme)
            if theme_index >= 0:
                self.theme_combo.setCurrentIndex(theme_index)
                self.selected_theme = saved_theme

            # Эти настройки можно расширить, когда будут добавлены
            # другие элементы управления на соответствующие вкладки

            # Пример загрузки настройки типа boolean
            # show_tips = self.settings.value("show_tips", True, bool)
            # if hasattr(self, "show_tips_checkbox"):
            #     self.show_tips_checkbox.setChecked(show_tips)

            # Пример загрузки настройки типа int
            # auto_save_interval = self.settings.value("auto_save_interval", 5, int)
            # if hasattr(self, "auto_save_spinbox"):
            #     self.auto_save_spinbox.setValue(auto_save_interval)

        except Exception as e:
            import logging
            logging.getLogger(__name__).error(f"Ошибка при загрузке настроек: {str(e)}")
    ...
    def _on_theme_changed(self, index):
        ...
        if new_theme != self.selected_theme:
            ...
            self.theme_manager.switch_visual_theme(new_theme)
    ...
    def accept(self):
        ...
        if self.selected_theme != self.theme_manager.get_current_visual_theme():
            self.theme_manager.switch_visual_theme(self.selected_theme)
    ...
    def reject(self):
        ...
        if self.selected_theme != self.theme_manager.get_current_visual_theme():
            self.theme_manager.switch_visual_theme(self.theme_manager.get_current_visual_theme())
