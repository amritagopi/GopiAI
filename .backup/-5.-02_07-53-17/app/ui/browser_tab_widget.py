...
def generate_browser_qss(theme_manager):
    """
    Генерирует QSS стили для браузерного интерфейса, соответствующие текущей теме.

    Args:
        theme_manager: Экземпляр ThemeManager для получения цветов темы

    Returns:
        str: Строка со стилями QSS
    """
    # Получаем основные цвета из темы
    bg_color = theme_manager.get_color('background', '#2D2D2D')
    text_color = theme_manager.get_color('foreground', '#EAEAEA')
    secondary_bg = theme_manager.get_color('secondary_background', '#3C3C3C')
    border_color = theme_manager.get_color('border', '#444444')
    accent_color = theme_manager.get_color('accent', '#3498db')
    hover_color = theme_manager.get_color('button_hover', '#4FA7E0')
    input_bg = theme_manager.get_color('input_background', '#3C3C3C')
    input_border = theme_manager.get_color('input_border', '#555555')
...
class BrowserTab(QWidget):
    ...
    def _on_loading_state_changed(self, is_loading, can_go_back, can_go_forward):
        ...
        if is_loading:
            ...
            # Показываем прогресс-бар и начинаем анимацию загрузки
            self.progress_bar.show()
            self.progress_bar.setValue(10)  # Начальное значение

            # Имитируем процесс загрузки с более плавной анимацией
            steps = 10
            for i in range(1, steps + 1):
                progress = int(10 + (90 * i / steps))
                QTimer.singleShot(i * 300 // steps, lambda p=progress: self.progress_bar.setValue(p))

            # Обновляем состояние URL-бара, чтобы показать, что страница загружается
            current_url = self.url_edit.text()
            if current_url and not current_url.startswith("⟳ "):
                self.url_edit.setText(f"⟳ {current_url}")

            # Вызываем сигнал обновления прогресса, если он установлен
            ...
            # Устанавливаем прогресс-бар на 100% и скрываем его через задержку
            self.progress_bar.setValue(100)
            QTimer.singleShot(500, lambda: self.progress_bar.hide())

            # Убираем индикатор загрузки из URL-бара
            current_url = self.url_edit.text()
            if current_url and current_url.startswith("⟳ "):
                self.url_edit.setText(current_url[2:])
...
class QWebEnginePage:
    """Заглушка для QWebEnginePage для совместимости с другими бэкендами браузера."""

    # Константы для совместимости с QWebEnginePage
    ...
    # Обработчик навигации по умолчанию
    def acceptNavigationRequest(self, url, _type, isMainFrame):
        """Стандартный обработчик навигации.

        Args:
            url: URL запроса
            _type: Тип навигации
            isMainFrame: Флаг, указывающий, является ли запрос для основного фрейма

        Returns:
            bool: True если навигация разрешена, False если запрещена
        """
        # В этой заглушке всегда разрешаем навигацию
        return True

    # Дополнительные методы для совместимости
    def runJavaScript(self, script, callback=None):
        """Запускает JavaScript код в контексте страницы.

        Args:
            script: Код JavaScript для выполнения
            callback: Функция обратного вызова для получения результата
        """
        # В этой заглушке ничего не делаем
        if callback:
            callback(None)  # Вызываем callback с None результатом
...
class MultiBrowserWidget(QWidget):
    ...
    tab_closed = Signal(int, QWidget)  # Сигнал закрытия вкладки (индекс, виджет)
    ...
    def close_tab(self, index):
        ...
        if self.tabs.count() > 1:
            ...
            if tab:
                # Эмитируем сигнал перед удалением вкладки, чтобы можно было очистить ресурсы
                self.tab_closed.emit(index, tab)
            ...
            if current_tab:
                # Эмитируем сигнал очистки текущей вкладки перед загрузкой новой страницы
                self.tab_closed.emit(0, current_tab)
