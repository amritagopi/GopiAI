...
from app.prompt.coding import CODING_AGENT_NEXT_STEP_PROMPT, CODING_AGENT_SYSTEM_PROMPT
...
class CodingAgent(ToolCallAgent):
    ...
    # Используем правильное имя поля tools_collection вместо coding_tools
    ...
    def __init__(self, **data):
        ...
        logger.info(
            f"Инициализирован {self.name} с {len(self.tools_collection.tools)} инструментами"
        )
    ...
    async def update_editor_state(self):
        ...
        try:
            ...
            code_control = self.tools_collection.get_tool("code_control")
            if code_control:
