...
class ToolCallAgent(ReActAgent):
    ...
    def add_tool(self, tool: Any) -> None:
        ...
        """
        self.tools_collection.add_tool(tool)
    ...
    async def think(self) -> bool:
        ...
        try:
            ...
            response = await self.llm.ask_tool(
                ...
                tools=self.tools_collection.to_params(),
    ...
    async def execute_tool(self, command: ToolCall) -> str:
        ...
        name = command.function.name
        if name not in self.tools_collection.tool_map:
        ...
        try:
            ...
            result = await self.tools_collection.execute(name=name, tool_input=args)
    ...
    def _is_special_tool(self, name: str) -> bool:
        ...
        for special_name in self.special_tool_names:
            # Проверка для Terminate
            if special_name == "terminate" and lower_name == "terminate":
                return True
            # Общая проверка
            if lower_name == special_name.lower():
                return True
        return False
    ...
    async def run(self, input_data: dict) -> Any:
        ...
        if isinstance(input_data, dict):
            ...
            # Initialize the agent's memory with the task
            self.memory.add_message(
            ...
            # If input is not a dict, treat it as a task string
            self.memory.add_message(Message.user_message(str(input_data)))
            result = await self.step()
            return result

    async def step(self, input_data: Optional[dict] = None) -> str:
        """Execute a single step in the agent's workflow."""
        if input_data:
            # Add input data to memory if provided
            task = input_data.get("task", "")
            context = input_data.get("context", {})
            self.memory.add_message(
                Message.user_message(
...
class ReactAgent(ToolCallAgent):
    ...
    agent_description: str = (
        "an agent that implements the ReAct paradigm with tool handling capabilities."
    )
