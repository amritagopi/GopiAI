...
from PySide6.QtWidgets import (
    ...
    QScrollArea, QFrame, QApplication, QDialog, QSizePolicy
)
...
class FlowNodeWidget(QFrame):
    ...
    def __init__(self, name, node_type="agent", parent=None):
        ...
        # Устанавливаем цвет фона в зависимости от типа узла
        if node_type == "agent":
            self.setStyleSheet("background-color: #cce6ff;")
        elif node_type == "tool":
            self.setStyleSheet("background-color: #e6ffcc;")
        elif node_type == "flow":
            self.setStyleSheet("background-color: #ffe6cc;")
        else:
    ...
    def _setup_ui(self):
        ...
        # Метка с типом узла
        type_label = QLabel(self.node_type.capitalize())
...
class FlowVisualizer(QWidget):
    ...
    def _setup_ui(self):
        ...
        # Заголовок
        header_label = QLabel("Flow Structure Visualization")
...
def show_flow_visualizer_dialog(flow, parent=None):
    """Показывает диалог с визуализацией потока."""
    dialog = QDialog(parent)
    dialog.setWindowTitle("Flow Visualizer")
    dialog.resize(800, 600)

    layout = QVBoxLayout(dialog)
    layout.setContentsMargins(0, 0, 0, 0)

    flow_visualizer = FlowVisualizer(dialog)
    flow_visualizer.set_flow(flow)

    layout.addWidget(flow_visualizer)
