...
Содержит настройки для различных компонентов приложения.
"""
...
# Импортируем BrowserConfig из app.config.__init__
try:
    from app.config.__init__ import BrowserConfig, ProxyConfig
except ImportError:
    # Используем заглушки, если импорт не удался
    class ProxyConfig:
        pass
...
class LLMSettings(BaseModel):
    ...
    vision: bool = Field(False, description="Whether the model supports vision")
...
class BrowserSettings(BaseModel):
    ...
    chrome_instance_path: Optional[str] = Field(None, description="Path to Chrome instance")
...
class Config:
    ...
    def _load_initial_config(self):
        ...
        default_settings = {
            ...
            "vision": base_llm.get("vision", False),
        }
        ...
        if tavily_config and tavily_config.get("api_key"):
            tavily_settings = TavilySettings(**tavily_config)
    ...
    @property
    def tavily(self) -> Optional[TavilySettings]:
        """Возвращает настройки Tavily"""
        try:
            return self._config.tavily
