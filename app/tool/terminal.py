...
"""
    ...
    # Функция обратного вызова для отображения в UI
    ui_callback: Optional[Callable[[str, str, str], None]] = None

    def set_ui_callback(self, callback: Callable[[str, str, str], None]):
        """
        Устанавливает функцию обратного вызова для отображения вывода команды в UI.

        Аргументы:
            callback: Функция с аргументами (command, stdout, stderr)
        """
    ...
    async def execute(self, command: str) -> CLIResult:
        ...
        for cmd in commands:
            ...
            # Вызов UI callback, если он установлен
            if self.ui_callback:
                self.ui_callback(sanitized_command, result.output, result.error)
    ...
    async def _handle_cd_command(self, command: str) -> CLIResult:
        """
        Handle 'cd' command by updating the current path.

        Args:
            command (str): The 'cd' command to execute.
        ...
        try:
            ...
            if not os.path.isabs(dir_path):
                dir_path = os.path.normpath(os.path.join(self.current_path, dir_path))

            # Check if directory exists
            if not os.path.isdir(dir_path):
                return CLIResult(output="", error=f"cd: {dir_path}: No such directory")
            ...
            if self.ui_callback:
                self.ui_callback(command, "", "")

            return CLIResult(output="", error="")
        except Exception as e:
            return CLIResult(output="", error=f"cd: {str(e)}")
