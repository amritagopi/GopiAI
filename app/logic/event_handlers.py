...
from app.ui.i18n.translator import tr # Added tr

def on_file_double_clicked(main_window, file_path):
    if not os.path.isfile(file_path):
        # Optionally, show a message to the user or log this
        QMessageBox.warning(main_window, "File Not Found", f"The file {file_path} does not exist.")
        return
...
def on_project_tree_double_clicked(main_window, index):
    try:
        if not hasattr(main_window, "project_explorer"):
            return
        file_path = main_window.project_explorer.tree_model.filePath(index)
        if not file_path:
            return
        ...
        if file_info.isDir():
            ...
            if hasattr(main_window, "_open_file"):
                ...
                logger.warning("MainWindow does not have _open_file method for project tree.")
    except Exception as e:
        logger.error(f"Error handling project tree double click: {str(e)}") # Changed print to logger.error

def on_dock_visibility_changed(main_window, visible):
    # Check if main_window and its relevant attributes/methods exist before calling
    if main_window and hasattr(main_window, "_update_view_menu") and callable(main_window._update_view_menu):
...
# It's good practice to have a logger instance available if you use logger.info/error etc.
import logging
