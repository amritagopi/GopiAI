...
def on_language_changed_event(main_window, language_code):
    try:
        ...
        for child in main_window.findChildren(QWidget):
            if hasattr(child, "windowTitle") and hasattr(child, "objectName"):
                ...
                if obj_name and not obj_name.startswith("qt_"): # Avoid Qt internal widgets
                    ...
                    if "project" in obj_name.lower(): title_key = "dock.project_explorer"
                    elif "chat" in obj_name.lower(): title_key = "dock.chat"
                    elif "terminal" in obj_name.lower(): title_key = "dock.terminal"
                    # Add more specific key mappings if needed

                    original_title = child.windowTitle() # Get current title as default if key not found
            ...
            if isinstance(child, QLabel) and hasattr(child, "objectName"):
                ...
                if obj_name and not obj_name.startswith("qt_"):
                    ...
                    if obj_name == "status_label": label_key = "status.ready"
                    elif obj_name == "file_info_label": label_key = "status.file_info_default" # Assuming a default key
                    elif obj_name == "agent_status_label": label_key = "status.agent_default" # Assuming a default key
                    else: label_key = f"label.{obj_name.lower()}" # Generic key

                    original_text = child.text()
                    translated_text = tr(label_key, original_text if original_text else "") # Provide default if empty
                    if translated_text != child.text():
                        child.setText(translated_text)
