"""
Core MainWindow class for GopiAI application.

This is the base class for the main window, without mixins.
The mixins are applied to this class in main_window.py.
"""
...
class MainWindowCore(QMainWindow):
    """Базовый класс главного окна приложения GopiAI."""
    ...
    def __init__(self, icon_manager, parent=None):
        ...
        self.status_bar = None  # Использование status_bar вместо statusBar!
    ...
    def _setup_ui(self):
        ...
        try:
            ...
            if not hasattr(self, "_connect_global_ui_signals"):
                logger.error("Method _connect_global_ui_signals does not exist!")
            else:
                # Вызов может быть None
                logger.info(f"Type of _connect_global_ui_signals: {type(self._connect_global_ui_signals)}")
                if callable(self._connect_global_ui_signals):
                    # Подключаем глобальные сигналы UI
                    self._connect_global_ui_signals()
                else:
                    logger.error("_connect_global_ui_signals is not callable!")
            ...
            status = update_integration_status(self)
            save_integration_status_to_serena(self, status)
    ...
    def _apply_initial_layout(self):
        ...
        # Настраиваем ширину доков (уменьшаем размер боковых панелей)
        try:
            if (
                ...
                # Вычисляем ширину для каждой панели - уменьшаем их размер
                # Теперь на боковые панели выделяем лишь 20% от ширины окна,
                # чтобы оставить больше места для редактора
                side_panel_width = int(window_width * 0.20)  # 20% ширины для каждой панели

                # Устанавливаем размеры доков
                self.resizeDocks(
                    [self.project_explorer_dock, self.chat_dock],
                    [side_panel_width, side_panel_width],
                    Qt.Horizontal,
                )

                logger.info(f"Dock widths set to {side_panel_width}px each (20% of window width)")

            # Браузер настраиваем в правой части
            if hasattr(self, "browser_dock") and self.browser_dock:
                self.addDockWidget(Qt.RightDockWidgetArea, self.browser_dock)
        except Exception as e:
            logger.error(f"Error setting up initial dock sizes: {e}")

        # Открываем Unified Chat View только если пользователь не отключил его специально
        ...
        try:
            ...
            if hasattr(self, "status_bar") and self.status_bar:
                self.status_bar.showMessage(
                    tr("status.auto_connect_signals", "Подключение сигналов..."), 3000
                )
            ...
            # Логируем результаты
            logger.info(f"Auto-connected {connected_count} signals")

            # Возвращаем результаты для использования в производных классах
    ...
    def open_color_harmony_dialog(self):
        """Открывает диалог настройки комплементарной цветовой гармонии."""
        try:
            if not has_color_harmony_dialog:
                logger.error("ColorHarmonyDialog не найден. Убедитесь, что minimal_version/color_harmony_dialog.py существует.")
                QMessageBox.warning(
                    self,
                    tr("error.title", "Ошибка"),
                    tr("main_window.color_harmony.dialog_import_error", "Не удалось открыть диалог цветовой гармонии.")
                )
                return

            # Создаем и открываем диалог
            dialog = ColorHarmonyDialog(self)
            dialog.exec()
            logger.info("Диалог цветовой гармонии открыт и закрыт.")

            # Обновляем меню тем, если оно существует
            if hasattr(self, '_update_themes_menu') and callable(self._update_themes_menu):
                self._update_themes_menu()
        except Exception as e:
            logger.error(f"Ошибка при открытии диалога цветовой гармонии: {e}")
            QMessageBox.critical(
                self,
                tr("error.title", "Ошибка"),
                f"{tr('main_window.color_harmony.open_error_prefix', 'Не удалось открыть диалог цветовой гармонии:')} {e}"
            )

    # Заглушки для методов из миксинов, которые вызываются в _setup_ui

    def _show_tab_context_menu(self, pos):
        """Заглушка для метода из TabManagementMixin."""
        logger.error("_show_tab_context_menu called on MainWindowCore, should be overridden by mixin")
        pass

    def _show_project_tree_context_menu(self, position):
        """Заглушка для метода из TabManagementMixin."""
        logger.error("_show_project_tree_context_menu called on MainWindowCore, should be overridden by mixin")
        pass

    def _update_view_menu(self):
        """Заглушка для метода из ViewManagementMixin."""
        logger.error("_update_view_menu called on MainWindowCore, should be overridden by mixin")
        pass

    def show_coding_agent_dialog(self):
        """Заглушка для метода из AgentIntegrationMixin."""
        logger.error("show_coding_agent_dialog called on MainWindowCore, should be overridden by mixin")
        pass
