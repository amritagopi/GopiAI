...
from PySide6.QtWidgets import (
    ...
    QFrame, QScrollArea, QComboBox, QToolButton,
    QSizePolicy, QMenu, QToolBar, QInputDialog,
...
class ThoughtTreeItem(QTreeWidgetItem):
    ...
    def __init__(
        ...
        self.setData(0, Qt.UserRole, node.node_id)
...
class ThoughtDetailView(QWidget):
    ...
    def _setup_ui(self):
        ...
        self.content_text.setStyleSheet("background-color: rgba(0,0,0,0.05); border: 1px solid rgba(0,0,0,0.1);")
        ...
        self.metadata_text.setStyleSheet("background-color: rgba(0,0,0,0.05); border: 1px solid rgba(0,0,0,0.1);")
    ...
    def set_node(self, node: Optional[ThoughtNode]):
        ...
        self.title_label.setText(f"{tr('thought_tree.detail.title', 'Thought Details')}: {node.node_type}")
        ...
        if node.metadata:
            ...
            self.metadata_text.setText(tr("thought_tree.detail.no_metadata", "No metadata"))
...
class ThoughtTreeWidget(QWidget):
    ...
    def _setup_ui(self):
        ...
        # Действия
        self.expand_all_action = QAction(get_icon("expand"), tr("thought_tree.expand_all", "Expand All"), self)
        self.collapse_all_action = QAction(get_icon("collapse"), tr("thought_tree.collapse_all", "Collapse All"), self)
        self.refresh_action = QAction(get_icon("refresh"), tr("thought_tree.refresh", "Refresh"), self)
        self.save_action = QAction(get_icon("save"), tr("thought_tree.save", "Save Tree"), self)
        self.load_action = QAction(get_icon("open"), tr("thought_tree.load", "Load Tree"), self)
        ...
        self.filter_combo.addItem(tr("thought_tree.filter.thought", "Thoughts"), "thought")
        self.filter_combo.addItem(tr("thought_tree.filter.decision", "Decisions"), "decision_point")
        self.filter_combo.addItem(tr("thought_tree.filter.question", "Questions"), "question")
        self.filter_combo.addItem(tr("thought_tree.filter.conclusion", "Conclusions"), "conclusion")
        ...
        self.tree_widget.setHeaderLabels([tr("thought_tree.column.content", "Content"),
    ...
    def refresh_tree(self):
        ...
        root_item = self._create_tree_item(
            ...
            is_current=self.thought_tree.current_node_id == self.thought_tree.root.node_id
        )
    ...
    def _populate_children(self, parent_item: QTreeWidgetItem, parent_id: str):
        ...
        """
        if parent_id not in self.thought_tree.nodes:
            return
            ...
            if child_id in self.thought_tree.nodes:
                ...
                if self._should_show_node(child_node):
                    child_item = self._create_tree_item(
                        child_node,
                        is_current=self.thought_tree.current_node_id == child_id
                    )
        ...
        for alt_id in parent_node.alternatives:
            if alt_id in self.thought_tree.nodes:
                ...
                if self._should_show_node(alt_node):
                    alt_item = self._create_tree_item(
                        alt_node,
                        is_current=self.thought_tree.current_node_id == alt_id,
                        is_alternative=True
                    )
    ...
    def _save_tree(self):
        ...
        try:
            ...
            QMessageBox.information(
                ...
                tr("thought_tree.save.success.message", "Thought tree saved successfully.")
            )
            ...
            QMessageBox.critical(
                self,
                tr("thought_tree.save.error.title", "Save Error"),
                tr("thought_tree.save.error.message", "Error saving thought tree: {0}").format(str(e))
            )
    ...
    def _load_tree(self):
        ...
        try:
            ...
            QMessageBox.information(
                ...
                tr("thought_tree.load.success.message", "Thought tree loaded successfully.")
            )
            ...
            QMessageBox.critical(
                ...
                tr("thought_tree.load.error.message", "Error loading thought tree: {0}").format(str(e))
            )
