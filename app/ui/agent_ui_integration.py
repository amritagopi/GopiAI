...
def integrate_browser_agent(main_window: QMainWindow) -> bool:
    """
    Интегрирует браузерного агента с интерфейсом.
    ...
    try:
        ...
        # Проверяем наличие диалога браузерного агента
        if not hasattr(main_window, "browser_agent_dialog") or not main_window.browser_agent_dialog:
            logger.info("Browser agent dialog not available, will be created when needed")
            return True

        # Получаем интерфейс браузерного агента
        if hasattr(main_window.browser_agent_dialog, "agent_interface"):
            agent_interface = main_window.browser_agent_dialog.agent_interface

            # Убеждаемся, что у интерфейса браузерного агента есть виджет браузера
            if hasattr(main_window.browser_agent_dialog, "browser_widget"):
                agent_interface.browser_widget = main_window.browser_agent_dialog.browser_widget
                logger.info("Connected browser agent interface to browser widget")

            # Подключаем сигналы чата в диалоге к агенту
            if hasattr(main_window.browser_agent_dialog, "chat_history"):
                chat_widget = main_window.browser_agent_dialog.chat_history

                # Подключаем отправку сообщений чата к браузерному агенту
                if hasattr(chat_widget, "message_sent"):
                    chat_widget.message_sent.connect(
                        lambda message: agent_interface.send_user_message(message)
                    )
                    logger.info("Connected browser agent chat widget to agent interface")
...
def update_integration_status(main_window: QMainWindow) -> Dict[str, bool]:
    ...
    try:
        ...
        # Проверяем интеграцию браузерного агента
