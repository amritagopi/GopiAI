...
from PySide6.QtWidgets import (
    ...
    QScrollArea, QSizePolicy, QMessageBox, QMenu, QInputDialog,
    QTabWidget, QListWidget, QListWidgetItem, QToolTip
)
from PySide6.QtCore import Qt, Signal, Slot, QTimer, QSize, QObject, QThread
from PySide6.QtGui import QIcon, QTextCursor, QFont, QColor, QTextCharFormat, QAction
...
from app.ui.thought_tree_widget import ThoughtTreeWidget
...
class SessionHistoryWidget(QWidget):
    ...
    def _setup_ui(self):
        ...
        refresh_btn.setToolTip(tr("reasoning_agent.sessions.refresh", "Refresh Sessions"))
        refresh_btn.clicked.connect(self.refresh_sessions)

        delete_btn = QPushButton(get_icon("delete"), "")
        delete_btn.setToolTip(tr("reasoning_agent.sessions.delete", "Delete Selected Session"))
    ...
    def _delete_selected_session(self):
        ...
        reply = QMessageBox.question(
            ...
            tr("reasoning_agent.sessions.delete.confirm.text", "Are you sure you want to delete this session?"),
    ...
    def _show_context_menu(self, position):
        ...
        # Действия
        load_action = QAction(tr("reasoning_agent.sessions.context.load", "Load Session"), self)
        rename_action = QAction(tr("reasoning_agent.sessions.context.rename", "Rename Session"), self)
        delete_action = QAction(tr("reasoning_agent.sessions.context.delete", "Delete Session"), self)
        ...
        if has_selection:
            ...
            load_action.triggered.connect(lambda: self.session_selected.emit(session_id))
...
class ReasoningAgentDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        self.save_session_button = QAction(get_icon("save"), tr("reasoning_agent.save_session", "Save Session"), self)
        ...
        self.save_session_button.setToolTip(tr("reasoning_agent.save_session.tooltip", "Save current session"))
        ...
        self.input_field.setPlaceholderText(tr("reasoning_agent.input.placeholder", "Enter your request here..."))
        ...
        self.tab_widget.addTab(thought_tree_tab, tr("reasoning_agent.tab.thought_tree", "Thought Tree"))
        self.tab_widget.addTab(history_tab, tr("reasoning_agent.tab.history", "History"))
    ...
    def _connect_signals(self):
        ...
        self.worker.execution_completed.connect(lambda: self._update_status(
    ...
    def _on_approve_clicked(self):
        ...
        self._append_output(
            tr("reasoning_agent.plan.approved", "Plan approved. Type a command or request to execute."),
            "green"
        )
    ...
    def _on_reject_clicked(self):
        ...
        self._append_output(
            tr("reasoning_agent.plan.rejected", "Plan rejected. Create a new plan or type a different request."),
            "red"
        )
    ...
    def _on_stop_clicked(self):
        ...
        self._append_output(
            tr("reasoning_agent.operation.cancelled", "Operation cancelled."),
            "orange"
        )
    ...
    def _on_save_session_clicked(self):
        ...
        if ok and title:
            ...
            if self.history_manager.save_session(self.current_session):
                self._append_output(
                    tr("reasoning_agent.session.saved", "Session saved: {0}").format(title),
                    "blue"
                )
    ...
    def _on_strategy_changed(self, index):
        ...
        if hasattr(self.agent, "set_strategy"):
            try:
                ...
                self._append_output(
                    tr("reasoning_agent.strategy.changed", "Reasoning strategy changed to: {0}").format(strategy),
                    "blue"
                )
            except Exception as e:
                logger.error(f"Error setting strategy: {str(e)}")
                self._append_output(
                    tr("reasoning_agent.strategy.error", "Error setting strategy: {0}").format(str(e)),
                    "red"
                )
    ...
    def _create_plan(self, task: str):
        ...
        self._append_output(
            tr("reasoning_agent.task", "Task: {0}").format(task),
            "blue"
        )

        # Обновляем статус
        self._update_status(tr("reasoning_agent.status.creating_plan", "Creating plan..."))
    ...
    def _execute_request(self, request: str):
        ...
        # Добавляем текст запроса в вывод
        self._append_output(
            tr("reasoning_agent.request", "Request: {0}").format(request),
            "blue"
        )
    ...
    def _on_worker_error(self, error: str):
        ...
        self._append_output(
            tr("reasoning_agent.error", "Error: {0}").format(error),
            "red"
        )
    ...
    def _on_plan_created(self, plan: str):
        ...
        self._append_output(
            tr("reasoning_agent.plan.created", "Plan created:"),
            "green"
        )
    ...
    def _on_thought_added(self, node: ThoughtNode):
        ...
        if self.current_session and hasattr(self.agent, "thought_tree") and self.agent.thought_tree:
            self.current_session.set_thought_tree(self.agent.thought_tree)
            self.history_manager.save_session(self.current_session)
    ...
    def _on_session_selected(self, session_id: str):
        ...
        if not session:
            self._append_output(
                tr("reasoning_agent.session.load_error", "Error loading session"),
                "red"
            )
            return
        ...
        self._append_output(
            tr("reasoning_agent.session.loaded", "Session loaded: {0}").format(session.title),
            "green"
        )
    ...
    def _on_session_deleted(self, session_id: str):
        ...
        if self.current_session and self.current_session.session_id == session_id:
            ...
            self._append_output(
                tr("reasoning_agent.session.new", "Started new session"),
                "green"
            )
    ...
    def _parse_plan(self, plan_text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for i, line in enumerate(lines):
                if line.lower().startswith("plan:") or line.lower().startswith("# plan"):
