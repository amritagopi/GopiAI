...
from PySide6.QtWidgets import (
    ...
    QTextEdit, QSplitter, QFrame, QToolButton, QScrollArea,
    QSizePolicy, QCheckBox, QToolTip
)
from PySide6.QtCore import Qt, Signal, Slot, QSize
from PySide6.QtGui import QIcon, QFont, QColor, QTextCharFormat, QTextCursor
...
class RiskLevelIndicator(QWidget):
    ...
    def __init__(self, level: str, parent=None):
        ...
        self.setToolTip(tr(f"plan_view.risk.{level}", f"{level.capitalize()} risk level"))
...
class PlanStepWidget(QWidget):
    ...
    def __init__(self, step_id: int, title: str, description: str,
                 risk_level: str = "low", tools: List[str] = None, parent=None):
        super().__init__(parent)
    ...
    def _setup_ui(self):
        ...
        risk_text = QLabel(tr(f"plan_view.risk.{self.risk_level}", f"{self.risk_level.capitalize()} risk"))
        risk_text.setStyleSheet(f"color: {risk_indicator.colors[self.risk_level].name()};")
        ...
        details_button.clicked.connect(lambda: self.details_requested.emit(self.step_id))
        ...
        description_text.setStyleSheet("background-color: rgba(0,0,0,0.05); border: 1px solid rgba(0,0,0,0.1);")
        ...
        if self.tools:
            ...
            for tool in self.tools:
                ...
                tool_label.setStyleSheet("background-color: rgba(0,0,0,0.08); padding: 2px 4px; border-radius: 3px;")
...
class PlanRisksWidget(QWidget):
    ...
    def _setup_ui(self):
        ...
        for risk, mitigation in self.risks.items():
            ...
            if "высок" in risk.lower() or "критич" in risk.lower() or "high" in risk.lower():
                risk_level = "high"
            elif "низк" in risk.lower() or "незначит" in risk.lower() or "low" in risk.lower():
                risk_level = "low"
...
class PlanApprovalWidget(QWidget):
    ...
    def _setup_ui(self):
        ...
        risk_checkbox = QCheckBox(tr("plan_view.approval.risks_acknowledged",
        ...
        approve_button.setStyleSheet("background-color: #4CAF50; color: white; padding: 8px 16px;")
        approve_button.setEnabled(False)  # Изначально отключена, пока не отметят чекбокс

        reject_button = QPushButton(tr("plan_view.approval.reject", "Reject Plan"))
        reject_button.setIcon(get_icon("plan_reject"))
        reject_button.setStyleSheet("background-color: #F44336; color: white; padding: 8px 16px;")
