    ...
    def analyze_qss_files(self, files):
        ...
        for i, file_path in enumerate(files):
            ...
            try:
                ...
                selectors = re.findall(r'([A-Za-z0-9#\.]+)\s*\{', content)
                duplicates = [item for item, count in {s: selectors.count(s) for s in selectors}.items() if count > 1]
                if duplicates:
                    joined_duplicates = ', '.join(duplicates[:5])
                    self.signals.message.emit(
...
class SimpleUIAuditor(QMainWindow):
    ...
    def export_report(self):
        ...
        file_path, _ = QFileDialog.getSaveFileName(self, "Сохранить отчет", "ui_audit_report.txt", "Text Files (*.txt)")
        if file_path:
            try:
